...
class SymmetryGroup:
    ...
    def find_subgroup_of_size(self,
                              k: int,
                              max_generator_size: int = 3,
                              min_generator_size: int = 1,
                              max_combinations: int = 200000) -> "SymmetryGroup | None":
        """
        Find a subgroup of order k by searching over small generating sets
        and delegating subgroup construction to create_induced_subgroup.

        Parameters
        ----------
        k : int
            Desired subgroup order (1 <= k <= |G|).
        max_generator_size : int
            Maximum size of generator sets to try.
        max_combinations : int
            Safety cap on number of generator combinations explored.

        Returns
        -------
        SymmetryGroup | None
            A subgroup of order k if found, else None.

        Notes
        -----
        - Uses create_induced_subgroup for closure; no manual closure logic here.
        - Stops at first matching subgroup.
        - For k = 1 returns the identity subgroup (if identity exists).
        - For k = |G| returns self.
        """
        from itertools import combinations
        if k < 1 or k > len(self):
            raise ValueError("k must be between 1 and the group order.")

        # Trivial cases
        if k == len(self):
            return self
        if k == 1:
            # Find identity element (matrix close to I)
            I = np.eye(self.dim)
            for idx in range(len(self)):
                if np.allclose(self.get_matrix(idx), I, atol=1e-12):
                    return create_induced_subgroup(self, [idx])
            # Fallback: just take first element (should behave like identity if representation valid)
            return create_induced_subgroup(self, [0])

        checked = 0
        # Try increasing generator set sizes
        for r in range(min_generator_size, max_generator_size + 1):
            for gens in combinations(range(len(self)), r):
                checked += 1
                if checked > max_combinations:
                    return None
                # Quick pruning: generator count cannot exceed desired size
                if len(gens) > k:
                    continue
                subgroup = create_induced_subgroup(self, list(gens))
                if len(subgroup) == k:
                    return subgroup
                # If subgroup larger than k we cannot shrink it; continue search
        return None
...
def create_induced_subgroup_from_permutations(parent_group: SymmetryGroup,
                                             generator_perms: List,
                                             verify: bool = True,
                                             tolerance: float = 1e-10,
                                             raise_on_fail: bool = True) -> SymmetryGroup:
    """
    Create the induced subgroup from a list of generator elements given directly
    as permutations (or the exact element objects used in parent_group.elements),
    instead of indices.

    This:
      1. Validates that each provided generator is an element of the parent group.
      2. Maps them to indices.
      3. Uses create_induced_subgroup to build the closure.
      4. (Optional) Verifies closure with verify_subgroup_closure.
    ...
    parent_group : SymmetryGroup
        The parent symmetry group.
    generator_perms : List
        List of generator elements (must exactly match entries in parent_group.elements).
        Examples:
          - For S_n: [(1,0,2,3), (0,2,1,3)]
          - For D_n: [('R', (1,2,0)), ('F', (0,2,1))]
    verify : bool
        If True, verify closure of the produced subgroup.
    tolerance : float
        Numerical tolerance passed to closure verification.
    raise_on_fail : bool
        If True, raise an error if verification fails; otherwise just warn.

    Returns
    -------
    SymmetryGroup
        The induced subgroup generated by the provided elements.

    Raises
    ------
    ValueError
        If a provided generator is not found in the parent group (or if
        verification fails and raise_on_fail is True).
    """
    import warnings

    # Map provided generators to their indices in the parent group
    element_index_map = {elem: idx for idx, elem in enumerate(parent_group.elements)}
    generator_indices = []
    for g in generator_perms:
        if g not in element_index_map:
            raise ValueError(f"Generator {g} not found in parent_group.elements.")
        generator_indices.append(element_index_map[g])

    # Build induced subgroup via existing function
    subgroup = create_induced_subgroup(parent_group, generator_indices)

    # Optional verification of closure
    if verify:
        ok = verify_subgroup_closure(subgroup, tolerance=tolerance)
        if not ok:
            msg = ("Closure verification FAILED for induced subgroup generated by "
                   f"{generator_perms}. Subgroup size={len(subgroup)}.")
            if raise_on_fail:
...
def create_subgroup_from_permutations(parent_group: SymmetryGroup,
                                    perms: List,
                                    verify: bool = False,
                                    tolerance: float = 1e-10,
                                    raise_on_fail: bool = True) -> SymmetryGroup:
    ...
    # Build induced subgroup via existing function
    subgroup = create_custom_group(
        [parent_group.elements[i] for i in generator_indices],
        dim=parent_group.dim,
        representation_map=parent_group.representation_map
    )
    ...
    if verify:
        ...
        if not ok:
            msg = ("Closure verification FAILED for induced subgroup generated by "
                   f"{perms}. Subgroup size={len(subgroup)}.")
            if raise_on_fail:
                raise ValueError(msg)
            else:
                warnings.warn(msg, RuntimeWarning)
